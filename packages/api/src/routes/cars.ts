...
export const carsRouter = router({
  ...
  //     // Validate car id type
  //     const validationResult = safeParse(pick(insertCarSchema, ['id']), input)
  ...
  //     const getCar = await db.select().from(CarTable).where(eq(CarTable.id, input.id))
  //     return getCar[0]
  //   }),
